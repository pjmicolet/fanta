...
TEST_CASE("Labels") {
  ...
  // START: should be at 0, but it's a label line so we might return -1 or 0 depending on impl
  // Based on your code: if (is_label_define(inst)) return -1;
  ...
  uint32_t expected_bne = Instructions::parse_one(Instructions::Registry::fetch("BNE").reg, -8);
  ...
  uint32_t expected_jmp = Instructions::parse_one(Instructions::Registry::fetch("JMP").reg, 0);
