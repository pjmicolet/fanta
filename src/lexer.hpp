  ...
  auto getToken() -> std::expected<Token, LexError> {
    ...
    while(cursor_ != end_) {
      switch(getType(body_.at(cursor_))) {
        case CharType::Alpha: return parseAlpha(cursor_);
        case CharType::Whitespace: cursor_++; break;
        case CharType::Symbol: return parseSymbol();
        case CharType::Num: return parseNumber(cursor_);
      }
    }
...
private:
  ...
  auto parseAlpha(std::size_t start) -> std::expected<Token, LexError> {
    while(cursor_< end_ && ::isalnum(body_[cursor_]) && (body_[cursor_] != ' ' || body_[cursor_] != '\0')) {
      cursor_++;
    }
    std::string_view lexeme = body_.substr(start, cursor_ - start);
    if(lexeme == "let") return Token{TokenType::KeywordLet, lexeme, 0 /*TODO: Add line*/, start, cursor_};
  ...
  auto isType(std::string_view lexeme) -> bool {
    return lexeme == "int" || lexeme == "float" || lexeme == "string" || lexeme == "long";
  }
  ...
  auto parseNumber(std::size_t start) -> std::expected<Token, LexError> {
    ...
    for(const auto& c : lexeme) {
      if(::isalpha(c)) return std::unexpected(LexError{LexErrorType::MalformedToken, 0, start, cursor_});
    }
    return Token{TokenType::IntLiteral, lexeme, 0 /*TODO: Add line*/, start, cursor_};
  }
    ...
    switch(body_[cursor_]) {
      case '-': {
        ...
        if(next == '>') {
          auto start = cursor_;
          cursor_++;
          return Token{TokenType::Arrow, body_.substr(start, 2), 0, start, cursor_++};
        }
        return Token{TokenType::Minus, body_.substr(cursor_, 1), 0, cursor_, cursor_++};
      }
      case '+': return Token{TokenType::Plus, body_.substr(cursor_, 1), 0, cursor_, cursor_++};
      case '=': return Token{TokenType::Equal, body_.substr(cursor_, 1), 0, cursor_, cursor_++}; // Ugly hack but I need to increment cursor at some point
      case '*' : return Token{TokenType::Mult, body_.substr(cursor_, 1), 0, cursor_, cursor_++};
      case '/' : return Token{TokenType::Slash, body_.substr(cursor_, 1), 0, cursor_, cursor_++};
      case ';' : return Token{TokenType::SemiColon, body_.substr(cursor_, 1), 0, cursor_, cursor_++};
      case '(' : return Token{TokenType::OpenParam, body_.substr(cursor_, 1), 0, cursor_, cursor_++};
      case '{' : return Token{TokenType::OpenBrace, body_.substr(cursor_, 1), 0, cursor_, cursor_++};
  ...
  auto peek() const -> char {
    if(cursor_ + 1 < end_) return body_.at(cursor_ + 1);
    return '\0';
  }
