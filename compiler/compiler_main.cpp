...
auto main(int argc, char* argv[]) -> int {
    if (argc < 2) {
        std::println(std::cerr, "Usage: {} <input_file> [-o <output_file>] [--dump-ast] [--dump-ir]", argv[0]);
        return 1;
    }
    ...
    try {
        ...
        if (insts.empty()) {
            std::println(std::cerr, "Error: Compiled program yielded 0 instructions.");
            return 1;
        }
        ...
        if (!out) {
            std::println(std::cerr, "Error: Could not open output file '{}'", outputFile);
            return 1;
        }
        out.write(reinterpret_cast<const char*>(insts.data()), insts.size() * sizeof(uint32_t));

        std::println("Successfully compiled: {} -> {} ({} instructions, {} bytes)", 
                     inputFile, outputFile, insts.size(), insts.size() * sizeof(uint32_t));
